...
void 		UARTStringPut(const uint8_t *cMessage);
void 		UARTStringGet(uint32_t ui32Base,char *cMessage,const char Iden);
void		strlower(char * uart_receive_string);
...
int main(void)
{
  ...
  SysTickPeriodSet(ui32SysClock/SYSTICK_FREQUENCY);
	...
	while (1)
	{
		...
		strlower(uart_receive_string);
		
		if (strcmp(uart_receive_string,"at+class")==0)
		{
			*uart_receive_string='O'; // clear the array
			UARTStringPut(str1);
		}
	 
	  if (strcmp(uart_receive_string,"at+studentcode")==0)
		{
			*uart_receive_string='O'; // clear the array
		  UARTStringPut(str2);
		} 
...
void UART0_Handler(void)
{
  ...
  UARTIntClear(UART0_BASE, uart0_int_status);					//Clear the asserted interrupts
	...
	{
	UARTStringGet(UART0_BASE,uart_receive_string,'@'); // @ 是事先约定好的指令结束标志
  } 
}
...
void strlower(char * uart_receive_string)
{
	while(*(uart_receive_string) != '\0') {
		*uart_receive_string = tolower(*uart_receive_string);
		++uart_receive_string;
	}
}
