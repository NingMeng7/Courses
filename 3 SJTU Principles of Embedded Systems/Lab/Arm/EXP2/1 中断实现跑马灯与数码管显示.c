...
int main(void)
{
  ...
  SysTickPeriodSet(ui32SysClock/SYSTICK_FREQUENCY);	//	ÉèÖÃ¶¨Ê±ÖÐ¶ÏÆµÂÊÎªSYSTICK_FREQUENCY
	SysTickEnable();
	SysTickIntEnable();
  IntMasterEnable();		
	...
	while (1) {
		...
		if (systick_100ms_status) {
			...
			if (++i2c_flash_cnt >= I2C_FLASHTIME/100) {
				...
				result = I2C0_WriteByte(PCA9557_I2CADDR,PCA9557_OUTPUT,~rightshift);	//PCA9557µãÁÁLED,µÍµçÆ½ÓÐÐ§
...
{
/*
	description: ÀûÓÃ¶¨Ê±Æ÷¼ÆÊ±¸Ä±äÈ«¾Ö±äÁ¿À´ÊµÏÖ100msÒÔ¼°10msµÄ¼ÆÊ±
	Arg: void
	return void
*/
