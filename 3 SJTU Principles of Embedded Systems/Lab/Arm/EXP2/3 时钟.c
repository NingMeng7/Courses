...
uint8_t 	I2C0_WriteByte(uint8_t DevAddr, uint8_t RegAddr, uint8_t WriteData);	//	0~255
uint8_t 	I2C0_ReadByte(uint8_t DevAddr, uint8_t RegAddr);
void		S800_I2C0_Init(void);
uint8_t flag,ccnt;

//systick software counter define
volatile uint16_t systick_10ms_couter,systick_1000ms_couter,systick_200ms_couter;		
volatile uint8_t	systick_10ms_status,systick_1000ms_status,systick_200ms_status,systick_200ms_status_1;
...
int main(void)
{
	...
	while (1)
	{
		...
		if (systick_200ms_status_1) {
			systick_200ms_status_1 = 0;
					if(++time[1] == 10) {
						time[1] = 0;
						if(++time[0]==6) {
							time[0] = 0;
		...
		if (systick_10ms_status) {
			...
			if (++i2c_flash_cnt >= I2C_FLASHTIME/10) {
				...
				switch(cnt) {
					case 0:
					{
						result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[time[0]]);	
						result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,bit7[cnt]);	
						break;
					}
					case 1:
					{
						result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[time[1]]);	
						result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,bit7[cnt]);	
						break;
					}
					case 2:
					{
						result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[time[2]]);	
						result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,bit7[cnt]);	
						break;
					}
					case 3:
					{
						result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[time[3]]);
						result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,bit7[cnt]);	
						break;
					}
				}
...
void SysTick_Handler(void)
{
	...
	{
		systick_10ms_couter = SYSTICK_FREQUENCY/500;	
		systick_10ms_status = 1;
	}
	
	if (systick_200ms_couter)	
